...
export const allColumns: Array<MRT_ColumnDef<any>> = [
   ...
   },
   {
      ...
      size: 60,
      Cell: ({ row }) => {
         const requestDate = row.original?.datesend;
   ...
   },
   {
      ...
      size: 200,
      Cell: ({ row }) => {
         const quantitySends = row?.original?.quantitySends;
     
         if (!quantitySends || quantitySends.length === 0) {
           return <Typography fontSize={14}>No Blood Type Available</Typography>;
         }
     
         // Extract and format blood types with their quantities
         const bloodTypeQuantities = quantitySends.map(item => {
